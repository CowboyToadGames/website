...
const richTextToHtml = (richTextDocument) => {
  ...
  const nodeRenderers = {
    ...
    "unordered-list": (node) =>
      `<ul>${node.content.map(renderNode).join("")}</ul>`,
    "ordered-list": (node) =>
      `<ol>${node.content.map(renderNode).join("")}</ol>`,
    "list-item": (node) => `<li>${node.content.map(renderNode).join("")}</li>`,
    "blockquote": (node) =>
      `<blockquote>${node.content.map(renderNode).join("")}</blockquote>`,
    "hr": () => "<hr>",
    "hyperlink": (node) =>
...
const transformEntry = (entry, includes, fields = []) => {
  ...
  fields.forEach((field) => {
    switch (field) {
      case "summary":
        base.summary = entry.fields.summary;
        break;
      case "image":
        base.image = resolveAsset(entry.fields.image, includes);
        break;
      case "game":
        base.game = entry.fields.game;
        break;
      case "date":
        base.date = entry.fields.date?.split("T")[0] ||
          entry.sys.createdAt?.split("T")[0];
        break;
    }
  });
  ...
  load: async () => {
    ...
    try {
      ...
      contentManager.content = {
        news: newsEntries.items.map((entry) =>
          transformEntry(entry, newsEntries.includes, [
            "summary",
            "image",
            "game",
            "date",
        ...
        faq: faqEntries.items.map((entry) =>
          transformEntry(entry, faqEntries.includes, ["game"])
        ),
        policies: policyEntries.items.map((entry) =>
          transformEntry(entry, policyEntries.includes, ["date"])
        ),
      };
...
const modal = {
  ...
  setupScrollFade: (dialog) => {
    ...
    const updateFade = () => {
      const isAtBottom = modalContent.scrollHeight - modalContent.scrollTop <=
